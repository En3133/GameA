...
class Sprite():
	...
	def loadAnimation(self, sheetWidth, sheetHeight, cellWidth, cellHeight):
		...
		self.boundX = cellWidth
		self.boundY = cellHeight
		self.animationCell = 0
		self.test = 0
	...
	def collidesWith(self, sprite):
		...
		if self.isVisible:
			if sprite.visible:
				# check borders
				left = self.x - (self.boundX / 2)
				top = self.y - (self.boundY / 2)
				right = self.x + (self.boundX / 2)
				bottom = self.y + (self.boundY / 2)
				spriteLeft = sprite.x - (sprite.boundX / 2)
				spriteTop = sprite.y - (sprite.boundY / 2)
				spriteRight = sprite.x + (sprite.boundX / 2)
				spriteBottom = sprite.y + (sprite.boundY / 2)
